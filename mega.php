...
class MEGA {
  ...
  /* Shared Keys */
  private $shared_keys = NULL;

  /* Default IDs */
  private $root_id = NULL;
  private $inbox_id = NULL;
  private $trashbin_id = NULL;
  ...
  public function node_list($handle = NULL, $args = array()) {
    ...
    if (isset($res['f'])) {
      ...
      $nodes = &$res['f'];
      $shared_keys = array();
      $this->_init_shared_keys($res, $shared_keys);
      foreach ($nodes as $index => $node) {
        $processed_node = $this->process_file($node, $this->shared_keys);
        if ($node['a']) {
          if ($processed_node['a']) {
            $result_node[$node['h']] = $processed_node;
          }
        }
      }
      return $result_node;
  ...
  public function node_find_path_descriptor($paths, $files=array()) {

    $files = $files ?: $this->node_list();
    $parent_desc = $this->root_id;
    $found = false;

    foreach($paths as $foldername) {
      if ($foldername != '') {
        foreach($files as $index => $file) {
          if ($file['a'] && $file['t'] && $file['a']['n'] == $foldername) {
  ...
  public function node_find($filename = null, $handle = null, $excluded_deleted = false) {
    $files = $this->node_list();
    if ($handle) {
      return $files[$handle];
    }
    ...
    foreach($files as $index => $file) {
      ...
      try {
        if ($parent_dir_name) {
          $parent_node_id = $this->node_find_path_descriptor($parent_dir_name, $files);
          if (
            $filename &&
            $parent_node_id &&
            $file['a'] &&
            $file['a']['n'] == $filename &&
            $parent_node_id == $file['p']
          ) {
            if ($excluded_deleted && $this->_trash_folder_node_id == $file['p']) {
              continue;
            }
            return $file;
          }
        }
        elseif ($filename && $file['a'] && $file['a']['n'] == $filename) {
          if ($excluded_deleted && $this->_trash_folder_node_id == $file['p']) {
            continue;
          }
          return $file;
  ...
  protected function _init_shared_keys($files, $shared_keys) {
    $ok_dict = array();
    if (array_key_exists("ok", $files)) {
      foreach ($files['ok'] as $ok_item) {
        $shared_key = $this->new_node_decrypt_key($ok_item['k'], $this->u_k_aes);
        $ok_dict[$ok_item['h']] = $shared_key;
      }
    }
    if (array_key_exists("s", $files)) {
      foreach ($files['s'] as $s_item) {
        if (!array_key_exists($s_item['u'], $shared_keys)){
          $shared_keys[$s_item['u']] = array();
        }
        if (array_key_exists($s_item['h'], $ok_dict)) {
          $shared_keys[$s_item['u']][$s_item['h']] = $ok_dict[$s_item['h']];
        }
      }
    }
    $this->shared_keys = $shared_keys;
  }

  protected function new_node_decrypt_key($a, $m_key) {
    $key = MEGAUtil::base64_to_a32($a);
    $key = MEGACrypto::decrypt_key($m_key, $key);
    return $key;
  }

  protected function process_file($node, $shared_keys) {

    if ($node['t'] == 0 || $node['t'] == 1) {
      $keys = array();
      $tempkeys = explode('/', $node['k']);
      foreach ($tempkeys as $key) {
        if (strpos($key, ":")) {
          $keypart = explode(':', $key);
          $keys[$keypart[0]] = $keypart[1];
        }
      }
      ...
      if (array_key_exists($uid, $keys)) {
        $key = $this->new_node_decrypt_key($keys[$uid], $this->u_k_aes);
      }
      elseif (
        array_key_exists("su", $node) &&
        array_key_exists("sk", $node) &&
        strpos($node['k'], ":")
      ) {
        $shared_key = $this->new_node_decrypt_key($node['sk'], $this->u_k_aes);
        $key = $this->new_node_decrypt_key($keys[$node['h']], $shared_key);

        if (!array_key_exists($node["su"], $shared_keys)) {
          $shared_keys[$node["su"]] = array();
        }
        $shared_keys[$node['su']][$node['h']] = $shared_key;
      }
      elseif ($node["u"] && array_key_exists($node["u"], $shared_keys)) {
        foreach ($shared_keys[$node['u']] as $hkey) {
          $shared_key = $shared_keys[$node['u']][$hkey];
          if (array_key_exists($hkey, $keys)){
            $key = $keys[$hkey];
            $key = $this->new_node_decrypt_key($key, $shared_key);
          }
        }
      }

      if (array_key_exists("EXP", $shared_keys)) {
        $temp_exp_keys = $shared_keys["EXP"];
      }
      else {
        $temp_exp_keys = array();
      }

      if ($node['h'] && array_key_exists($node['h'], $temp_exp_keys)) {
        $shared_key = $temp_exp_keys[$node['h']];
        $temp_key = end(explode(":", $node['k']));
        $encrypted_key = MEGAUtil::base64urldecode($temp_key);
        $encrypted_key = MEGAUtil::str_to_a32($encrypted_key);
        $key = $this->new_node_decrypt_key($encrypted_key, $shared_key);
        $node['shared_folder_key'] = $shared_key;
      }
      ...
      if ($key !== null) {
        ...
        if ($node['t'] == 0) {
          ...
          $node['iv'] = array($key[4], $key[5], 0, 0);
          $node['meta_mac'] = array($key[6], $key[7]);
        }
        # folder
        else {
          $k = $key;
        ...
        $node['key'] = $key;
        $node['k'] = $k;
        $attributes = MEGAUtil::base64urldecode($node['a']);
        $attributes = MEGACrypto::dec_attr($attributes, $k);
        $node['a'] = $attributes;
      }
      elseif ($node['k'] == '') {
        $node['a'] = false;
    ...
    elseif ($node['t'] == 2) {
      $this->root_id = $node['h'];
      $node['a'] = array('n' => 'Cloud Drive');
    }

    elseif ($node['t'] == 3) {
      $this->inbox_id = $node['h'];
      $node['a'] = array('n' => 'Inbox');
    }

    elseif ($node['t'] == 4) {
      $this->trashbin_id = $node['h'];
      $node['a'] = array('n' => 'Rubbish Bin');
    }
    return $node;
  }
   ...
   */
  public function node_publish($node) {
    ...
    $ph = array_shift($res);
    $decrypted_key = MEGAUtil::a32_to_base64($node['key']);
  ...
  public function user_login_session($email, $password, $args = array()) {
    ...
    $req = array('a' => 'us0', 'user' => $email);
    $get_user_salt_resp = $this->api_req(array($req));

    if (!array_key_exists("s", $get_user_salt_resp[0])) {
      ...
      $this->log("Calculating user key and hash for v2 account...");
      $user_salt = MEGAUtil::base64_to_a32($get_user_salt_resp[0]['s']);
      $pbkdf2_key = openssl_pbkdf2(
        utf8_encode($password),
        MEGAUtil::a32_to_str($user_salt),
        32,
        100000,
        "sha512"
      );
      $pk = substr($pbkdf2_key, 0, 16);
      $uh = MEGAUtil::base64urlencode(substr($pbkdf2_key, 16, 16));
    }
  ...
  protected function file_download_url($url, $size, $key, $dest) {
    ...
    $ret = 0;
    $buffer = '';
    // Tempfile to write encrypted data
    ...
    foreach ($chunks as $chunk_start => $chunk_size) {
      ...
      if ($bytes = fwrite($temp_file, $chunk)) {
        $ret += $bytes;
      }
    }
    ...
    // Open Encrypted file and decrypt to actual location
    $enc_data = file_get_contents($temp_filename);
    $dec_data = openssl_decrypt($enc_data, 'aes-128-ctr', $aeskey, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
    fwrite($dest, $dec_data);
  ...
  public static function parse_link($link, $component = NULL) {
    $fragment = parse_url($link, PHP_URL_PATH)."#".parse_url($link, PHP_URL_FRAGMENT);
    if (empty($fragment)) {
      return FALSE;
    }
    $matches = array();
    if (preg_match('/^\/(file|folder)\/([a-zA-Z0-9]+)#(?:([a-zA-Z0-9_,\\-]+))?/', $fragment, $matches)) {
